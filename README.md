...
Working on it! But the general idea is to use the most expeditious command options, i.e. download and install ASAP with forced restart.
...
5. If the string in Jamf matches the one that was generated (and it's still valid), the service sends the `EraseDevice` command to that device via the Jamf API
...
- Deployment templates/examples for some cloud providers
